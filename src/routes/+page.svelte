...
{#if standaloneSettings}
	<SettingsScreen standalone setupHighlight={standaloneSetupHighlight} onClose={closeStandaloneSettings} />
{:else}
	<main>
		{#if gateLoading}
		...
		{:else if gateError}
			...
			>
				...
				<button class="text-body-md text-on-surface/70 underline" onclick={refreshGate}>Retry</button>
			</div>
