...
- [Remove legacy voice purge from startup](./issues/22-remove-legacy-voice-purge-from-startup.md) — **closed**: `services/legacy_voice_purge.rs` and its call site deleted outright; only the unconditional Keychain-key delete (ticket 14) remains.
- [Reorder startup sequencing](./issues/24-reorder-startup-sequencing.md) — **closed**: `create_tray` now runs right after hotkeys/settings resolve; the model-seed integrity loop + availability warnings moved into a background `tauri::async_runtime::spawn`. `prewarm_dictate_window` left synchronous (main-thread-only API, didn't gate tray anyway) — flagged as a possible future ticket if `setup()`'s total time (not just tray-appearance time) matters later.
- [Cache VAD hash fingerprint](./issues/25-cache-vad-hash-fingerprint.md) — **closed**: new `dest_needs_bundle_restore_cached` in `bundled_models.rs` (mtime+size+expected-sha sidecar cache), wired into the VAD startup check. Built generically so [Cache Whisper hash](./issues/36-transcribe-cache-whisper-hash.md) can reuse it.
- [Parallelize Sortformer load](./issues/35-transcribe-parallelize-sortformer-load.md) — **closed**: implemented via `std::thread::scope` in `run_post_capture_transcription_with_inference` — Sortformer's on-demand diarize pass now runs on a scoped thread concurrently with the ASR pass instead of strictly after, since it shares no lock with `ModelService.inference_gate` (unlike ticket 34's case). Performance win is unverified (no real models/hardware in this environment to profile); correctness verified via existing tests.
- [Cache Whisper hash](./issues/36-transcribe-cache-whisper-hash.md) — **closed, already solved**: `ModelService::model_integrity_ok_cached` (in-memory mtime+size cache) already exists on `release/0.3` and is already wired into `transcribe_pcm_with_progress` — this must have landed independently since the ticket's stale research base. No code changed.
- [Prefetch next decode](./issues/37-transcribe-prefetch-next-decode.md) — declined: overlapping next-item decode with the current item's inference needs a real pipeline restructure (prefetch failure handling, cancellation semantics), with an expected win capped by decode time (likely small next to Whisper+diarization per item) and no real audio/models available here to measure or verify it. Revisit with real batch timing data first.
