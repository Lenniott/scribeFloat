...
<StepShell title="Grant permissions" subtitle="ScribeFloat needs microphone access to record. Accessibility lets Dictate paste text automatically.">
	{#snippet children()}
		<div class="space-y-2">
						...
						<div class="flex-1 min-w-0">
							<p class="sf-label-md text-fg">
								...
								{#if status.kind === "microphone"}
									<span class="sf-label-sm text-fg-muted ml-1">required</span>
								{:else}
									<span class="sf-label-sm text-fg-muted ml-1">optional</span>
								{/if}
							</p>
							{#if !status.granted && status.can_request}
								<p class="sf-body-md text-fg-dim mt-0.5">{PERMISSION_PRIMERS[status.kind] ?? ""}</p>
							{:else if status.granted && status.hint}
								<p class="sf-label-sm text-fg-dim mt-1">{status.hint}</p>
							{/if}
						</div>
						<div class="shrink-0">
							{#if status.granted}
							...
							{:else}
								<div class="flex items-center gap-1.5 text-fg-muted">
									<Circle class="size-4" />
									<span class="sf-label-sm">Not available</span>
								</div>
							{/if}
						</div>
					</div>
			...
			{#if !micGranted}
				<p class="sf-label-sm text-warning px-1">
