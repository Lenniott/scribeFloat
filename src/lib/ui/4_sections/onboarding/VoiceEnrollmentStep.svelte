<script lang="ts">
	...
	const safeToStop = $derived(speechS >= 5 && purity >= 0.5);
				...
				<div class="space-y-4">
					...
					<div class="space-y-2">
						<div class="flex items-center justify-between sf-label-sm text-fg-dim">
							<span>Purity {purityPct}%</span>
							<span>{statusText}</span>
						</div>
						<div class="h-2 overflow-hidden rounded-sm bg-fill">
							<div class="h-full bg-focus transition-[width] duration-200" style={`width:${purityPct}%`}></div>
						</div>
					</div>
					<div class="space-y-2">
						<div class="h-2 overflow-hidden rounded-sm bg-fill">
					...
					<p class="sf-label-sm text-fg-dim">Duration: {speechS.toFixed(1)} s</p>
				</div>
			{:else if step === 'naming'}
				<div class="space-y-3">
					<TextField
