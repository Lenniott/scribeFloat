...
fn copy_helper_if_changed(src: &std::path::Path, dest: &std::path::Path) {
    ...
    if dest.is_file() {
        ...
        if dest_bytes == src_bytes {
            ...
            .expect("restore helper executable permissions");
            return;
        }
    }
    // copy preserves executable permissions; write would create a non-executable file.
    std::fs::copy(src, dest).expect("copy set-default-output bundle binary");
}
