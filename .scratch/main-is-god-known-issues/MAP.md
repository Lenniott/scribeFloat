...
- [Triage: Record button context ambiguity](issues/z_11-record-button-context-ambiguity.md) — **Now**, done: label-only fix — `TitleBar.svelte` Record button now shows "Continue recording" vs "New recording" (with matching title/aria-label) reusing the existing route-match logic. Confirmation-modal variant not needed.
