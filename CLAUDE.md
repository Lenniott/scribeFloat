...
5. **Do not call `set_has_visible_windows(false)` at startup** or at any point when a window (especially an `always_on_top` HUD) may later call `set_focus()`. Use `LSUIElement = true` in `Info.plist` instead to suppress the Dock icon statically — no runtime activation policy change required.
