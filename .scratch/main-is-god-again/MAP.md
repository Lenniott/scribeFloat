...
- Load-performance effort (2026-07-23, "dictate process was slow" → root out the sequential-loading habit wherever it appears): startup, Dictate, and Scribe fixes implemented and committed to `release/0.3` (see Decisions so far). Transcribe cluster (35/36/37) still open. Whether Settings/onboarding hide the same pattern is still fog — not yet ticketed. Worktree-based parallel agents hit a stale-provisioning bug (worktrees forked from a commit 278 behind `release/0.3`); several tickets ended up hand-reconciled directly against current `HEAD` instead of merged from their worktree branch — noted per-ticket where that happened.
