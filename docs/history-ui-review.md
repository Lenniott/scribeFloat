...
> Read this before changing Transcripts list/detail screens or history components (`transcripts.svelte`, `HistoryDetailPane`, list cards, filter panel).

---

## Architecture

Transcripts lives in the main app shell (`transcripts.svelte` route inside `app-shell.svelte`). List and detail are **separate full-height modes** — no `SplitPane`.
...
- **Detail footer** uses `PanelFooter` (flex `shrink-0` below scroll). Do not add `FixedFooterBar` to detail.
...
- **Layout**: chrome `shrink-0`; one `ScrollablePanel` body per pane — see ui-enforcement layout-scroll reference.
