<script lang="ts">
	...
	let {
		...
		/** Whole card opens the note; no action icons or capture chip. */
		compact?: boolean;
	...
	const metaLine = $derived(
		`${formatShortDate(item.created_at)} · ${formatDurationFromSecs(item.duration_secs)}`,
	);

	const cardClass =
		'w-full rounded-md border border-fill bg-card p-4 text-left transition-colors hover:bg-fill/80';
</script>

{#snippet noteBody()}
	<div class="flex items-start gap-3">
		<SourceKindIcon origin={item.origin} />
		<div class="min-w-0 flex-1">
			<div class="flex items-baseline justify-between gap-2">
				{#if compact}
					<p class="min-w-0 flex-1 truncate sf-body-md-strong text-fg">{item.title || item.id}</p>
				{:else if onselect}
					<button
						type="button"
			...
			<div class="mt-1.5 flex flex-wrap items-center gap-2">
				...
				{#if item.tags && item.tags.length > 0}
					{#each (compact ? item.tags : item.tags.slice(0, 3)) as tag (tag)}
...
{#if compact && onselect}
	<button type="button" class={cardClass} {disabled} onclick={() => onselect()}>
		{@render noteBody()}
	</button>
{:else}
	<article class="{cardClass} {disabled ? 'pointer-events-none opacity-50' : ''}">
		{@render noteBody()}
	</article>
{/if}
