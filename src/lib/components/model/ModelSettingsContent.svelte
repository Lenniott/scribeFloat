<script lang="ts">
  ...
  async function onRefresh() {
    ...
    try {
      await modelStore.refresh();
      if (!modelStore.error && showToast)
        showToastMessage(toastMessages.modelsRefreshed);
    } finally {
      const elapsed =
        (typeof performance !== "undefined" ? performance.now() : Date.now()) -
        started;
  ...
  async function removeModel(modelId: string) {
    ...
    if (!modelStore.error && showToast)
      showToastMessage(toastMessages.modelRemoved);
  }
...
<div class="flex h-full min-h-0 flex-1 flex-col">
  {#if showHeader}
    <h2 class="sf-headline-sm shrink-0 p-4">{heading}</h2>
  {/if}

  {#if modelStore.error}
    <p
      class={`rounded-md border border-fill px-3 py-2 sf-body-md text-destructive ${showHeader ? "mx-4 mt-3" : "mx-4 mt-4"}`}
    >
      {modelStore.error}
    </p>
  {/if}
      ...
      >
        ...
        <div class="flex shrink-0 flex-col items-end gap-0.5">
          ...
          <select
            ...
            class="sf-body-md h-8 min-w-[10rem] max-w-[14rem] cursor-pointer truncate rounded-md border border-fill bg-panel py-2 pr-8 pl-2 text-fg disabled:cursor-not-allowed disabled:opacity-40"
            value={selectedId}
  ...
  <div class="min-h-0 flex-1 overflow-y-auto px-4 py-3">
    ...
    <div
      class="mt-2 divide-y divide-fill overflow-hidden rounded-md border border-fill bg-panel"
    >
      {#each modelStore.models as model (model.id)}
        <div class="flex items-center justify-between gap-3 px-3 py-2.5">
          <div class="min-w-0 flex-1 items-center">
            <div
              class="flex min-w-0 items-baseline gap-x-2 gap-y-0.5"
            >
              <span
